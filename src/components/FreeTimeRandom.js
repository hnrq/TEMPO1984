        ...
        return(
            <div ref={(div) => {this.lifespanTimer = div}} className={`text-container`} tabIndex="0">
                <div className={this.state.tip ? 'invisible' : ''}>
                    ...
                    <div style={{display:'block'}}><span className="category" onClick={() => {this.getTip(HABILITIES)}}><Typed stopped={true} showCursor={false} typedRef={(habilities) => {this.habilities = habilities}} typeSpeed={30} strings={['New skills']}/></span></div>
                </div>
                {this.renderTip()}
            </div>
        )
